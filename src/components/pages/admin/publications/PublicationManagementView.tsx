...
export function PublicationManagementView(props: Props) {
    ...
    return <Table.Root columns={columns} data={props.publications} getRowId={(item) => item.id.toString()} isFetching={props.isFetching} emptyState={hasFilters ? "Nenhuma publicação encontrada para os filtros atuais." : "Nenhuma publicação cadastrada."} pagination={{ pageIndex: props.pageIndex, pageSize: props.pageSize, pageCount: props.totalPages, rowCount: props.totalItems, onChange: props.onPagination }} footer={<Table.Pagination itemLabel="publicação" />}>
        ...
        <div className="border-b border-j-gray-200 p-4 md:px-6"><Table.Filters onClear={props.onClear} hasActiveFilters={hasFilters} className="flex-wrap">
            ...
            <Select.Unregister name="type" label="Tipo" labelClassName="text-j-gray-700" value={props.query.type ?? ""} onChange={(e) => props.onFilter({ type: (e.target.value || undefined) as PublicationType | undefined })} className={filterClass}><option value="">Todos</option><option value="event">Evento</option><option value="notice">Aviso</option><option value="service">Serviço</option></Select.Unregister>
            <Select.Unregister name="status" label="Status" labelClassName="text-j-gray-700" value={props.query.status ?? ""} onChange={(e) => props.onFilter({ status: (e.target.value || undefined) as PublicationStatus | undefined })} className={filterClass}><option value="">Todos</option><option value="draft">Rascunho</option><option value="published">Publicado</option><option value="archived">Arquivado</option></Select.Unregister>
            <Select.Unregister name="priority" label="Prioridade" labelClassName="text-j-gray-700" value={props.query.noticePriority ?? ""} onChange={(e) => props.onFilter({ noticePriority: (e.target.value || undefined) as NoticePriority | undefined })} className={filterClass}><option value="">Todas</option><option value="low">Baixa</option><option value="medium">Média</option><option value="high">Alta</option></Select.Unregister>
            <Select.Unregister name="category" label="Categoria" labelClassName="text-j-gray-700" value={props.query.serviceCategory ?? ""} onChange={(e) => props.onFilter({ serviceCategory: (e.target.value || undefined) as ServiceCategory | undefined })} className={filterClass}><option value="">Todas</option><option value="benefit">Benefício</option><option value="help">Ajuda</option><option value="general">Geral</option></Select.Unregister>
