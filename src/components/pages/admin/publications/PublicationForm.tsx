...
export function PublicationForm({ publication }: { publication?: Publication }) {
    ...
    return <form onSubmit={form.handleSubmit((value) => mutation.mutate(toInput(value)), (errors: FieldErrors<PublicationFormValues>) => { const first = Object.values(errors)[0]; toast.error(typeof first?.message === "string" ? first.message : "Revise os campos destacados."); })} noValidate className="flex w-full flex-col gap-6">
        <section className="grid grid-cols-1 gap-4 rounded-2xl border border-j-gray-200 bg-j-white p-5 shadow-sm md:grid-cols-2 md:p-6">
            <Select.Unregister label="Tipo" required labelClassName={label} className={field} error={error("type")} {...form.register("type")}><option value="event">Evento</option><option value="notice">Aviso</option><option value="service">Serviço</option></Select.Unregister>
            <Select.Unregister label="Status" required labelClassName={label} className={field} error={error("status")} {...form.register("status")}><option value="draft">Rascunho</option><option value="published">Publicado</option><option value="archived">Arquivado</option></Select.Unregister>
            ...
            {type === "notice" && <><Input label="Expiração" type="datetime-local" labelClassName={label} className={field} error={error("expiresAt")} {...form.register("expiresAt")} /><Select.Unregister label="Prioridade" labelClassName={label} className={field} error={error("noticePriority")} {...form.register("noticePriority")}><option value="low">Baixa</option><option value="medium">Média</option><option value="high">Alta</option></Select.Unregister></>}
            {type === "service" && <Select.Unregister label="Categoria" labelClassName={label} className={field} error={error("serviceCategory")} {...form.register("serviceCategory")}><option value="benefit">Benefício</option><option value="help">Ajuda</option><option value="general">Geral</option></Select.Unregister>}
        </section>
