...
interface RevokeUserRoleModalProps {
    userId: number;
    userName: string;
    roleId: number;
    roleName: string;
...
export function RevokeUserRoleModal({
    ...
    const mutation = useMutation({
        mutationFn: () => putUserRoleAction({
            id: userId,
            roles: { roleIds: currentRoleIds.filter((id) => id !== roleId) },
        }),
        onSuccess: () => {
            toast.success(`Cargo "${roleName}" revogado com sucesso!`);
            queryClient.invalidateQueries({ queryKey: ["authorization", "users", userId, "roles"] });
            backToRolesView();
        },
        onError: (error) => toast.error(error.message || "Erro ao revogar cargo."),
    });
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p className="mt-1 max-w-lg text-xs leading-relaxed text-j-gray-500 md:text-sm">
                            Tem certeza que deseja revogar o cargo{" "}
                            <span className="font-bold text-j-blue-800">{roleName}</span>{" "}
                            de <span className="font-bold text-j-blue-800">{userName}</span>?
...
export default RevokeUserRoleModal;
