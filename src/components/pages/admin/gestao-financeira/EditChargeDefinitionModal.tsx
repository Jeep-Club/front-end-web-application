...
import { getChargeDefinitionAction } from "@/actions/billing/chargeDefinitions/get";
import { updateChargeDefinitionAction } from "@/actions/billing/chargeDefinitions/update";
import { ChargeDefinitionFormFields } from "./ChargeDefinitionFormFields";

interface EditChargeDefinitionModalProps {
    chargeDefinitionId: number;
}

export function EditChargeDefinitionModal({ chargeDefinitionId }: EditChargeDefinitionModalProps) {
    const { setClose } = useModal();
    const queryClient = useQueryClient();

    const { data: chargeDefinition, isLoading } = useQuery({
        queryKey: ["billing", "charge-definitions", chargeDefinitionId],
        queryFn: () => getChargeDefinitionAction(chargeDefinitionId),
    });

    const mutation = useMutation({
        mutationFn: (data: ChargeDefinitionFormData) => updateChargeDefinitionAction(chargeDefinitionId, data),
        onSuccess: () => {
            toast.success("Definição de cobrança atualizada com sucesso!");
            queryClient.invalidateQueries({ queryKey: ["billing", "charge-definitions"] });
            setClose();
        },
        onError: (error) => toast.error(error.message || "Erro ao atualizar definição de cobrança."),
    });
        ...
        >
            ...
            <header className="border-b border-j-gray-200 px-5 pb-5 pr-16 pt-6 md:px-8 md:pb-6 md:pr-20 md:pt-8">
                <h2 className="text-xl font-extrabold text-j-blue-800 md:text-2xl">
                    Editar definição de cobrança
                </h2>
                <p className="mt-1 max-w-lg text-xs leading-relaxed text-j-gray-500 md:text-sm">
                    A alteração afeta apenas usos futuros desta definição.
                </p>
            </header>

            {isLoading || !chargeDefinition ? (
                ...
                <Form<ChargeDefinitionFormData>
                    ...
                    formOptions={{
                        defaultValues: {
                            name: chargeDefinition.name,
                            description: chargeDefinition.description ?? "",
                            defaultAmount: chargeDefinition.defaultAmount,
                            recurrenceType: chargeDefinition.recurrenceType,
                            required: chargeDefinition.required,
                            paymentAcceptancePolicy: chargeDefinition.paymentAcceptancePolicy,
                            latePaymentGraceDays: chargeDefinition.latePaymentGraceDays ?? undefined,
                        },
                    }}
...
export default EditChargeDefinitionModal;
